...
import { MoreVertical, Globe, ChevronDown, Smartphone, ShieldCheck, ArrowLeft, QrCode, Sparkles } from 'lucide-react'
...
export default function Login() {
  ...
  // Steps: 1 = Welcome, 2 = Phone Number, 3 = OTP / Verification, 4 = Companion Link QR
  ...
  return (
    <div className="relative min-h-screen w-screen flex items-center justify-center bg-[#eef7ef] overflow-hidden font-sans text-slate-800 selection:bg-emerald-200">
      {/* Decorative background shapes mimicking WhatsApp background */}
      ...
      {/* Main Container Card */}
      <div className="relative z-10 w-full max-w-[400px] min-h-[640px] mx-4 bg-white rounded-3xl shadow-xl border border-slate-100 flex flex-col justify-between overflow-hidden transition-all duration-300">
        ...
        {step === 1 && (
          <div className="flex-1 flex flex-col justify-between p-6 text-center animate-fadeIn">
            ...
            <div className="pt-4 pb-2">
              <button
                onClick={() => setStep(2)}
                className="w-full py-3.5 bg-[#00a884] hover:bg-[#008f70] text-white font-medium rounded-full shadow-md transition-all active:scale-[0.98] text-sm"
              >
                Agree and continue
              </button>
            </div>
            ...
            <div>
              <div className="flex justify-between items-center mb-6">
                ...
                <div className="relative">
                  ...
                  {showMenu && (
                    <div className="absolute right-0 top-8 w-56 bg-white rounded-xl shadow-lg border border-slate-100 py-2 z-50 animate-in fade-in zoom-in-95">
                      <button
                        onClick={() => {
                          setShowMenu(false)
                          setStep(4)
              ...
              <p className="text-xs text-slate-500 text-center mb-6 px-2 leading-relaxed">
                Rivo will need to verify your phone number. Carrier charges may apply.
              </p>
              ...
              <form onSubmit={handlePhoneSubmit} className="space-y-4 max-w-xs mx-auto relative">
                ...
                {/* Country Picker Modal / Dropdown List */}
                {showCountryPicker && (
        ...
        {step === 3 && (
          <div className="flex-1 flex flex-col justify-between p-6 animate-fadeIn">
            <div>
              ...
              <p className="text-xs text-slate-500 text-center mb-2 px-2 leading-relaxed">
                Waiting to automatically detect an SMS sent to <span className="font-semibold text-slate-700">{countryCode} {phoneNumber}</span>. <span className="text-emerald-600 hover:underline cursor-pointer" onClick={() => setStep(2)}>Wrong number?</span>
              </p>
              ...
              <div className="flex justify-center gap-2 my-8">
                {otp.map((digit, idx) => (
                  <input
                    ...
                    className="w-10 h-12 text-center text-xl font-bold border-b-2 border-slate-300 focus:border-emerald-500 bg-slate-50 rounded-t-md focus:outline-none transition"
                  />
                ))}
              </div>
              <p className="text-xs text-center text-slate-400">Enter any 6-digit code (e.g. 123456)</p>
            </div>
            ...
            <div className="pt-4 pb-2">
              <button
                ...
                disabled={loading}
                className="w-full py-3.5 bg-[#00a884] hover:bg-[#008f70] text-white font-medium rounded-full shadow-md transition-all active:scale-[0.98] text-sm disabled:opacity-50"
              >
        ...
        {/* STEP 4: LINK AS COMPANION DEVICE (QR CODE) */}
        {step === 4 && (
          <div className="flex-1 flex flex-col justify-between p-6 animate-fadeIn">
            <div>
              <div className="flex justify-between items-center mb-6">
            ...
            <div className="pt-4 pb-2">
              <button
                onClick={() => setStep(2)}
                className="w-full py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 font-medium rounded-full transition-all text-sm"
              >
                Back to Phone Input
              </button>
            </div>
